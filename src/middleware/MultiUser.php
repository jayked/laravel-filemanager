...
namespace Jayked\Laravelfilemanager\middleware;
...
class MultiUser
{
    ...
    {
        if(Str::startsWith($previous_dir, '/' . Config::get('lfm.shared_folder_name'))) {
            return true;
        }

        if(Str::startsWith($previous_dir, '/' . (string)Auth::user()->user_field)) {
            return true;
        }
