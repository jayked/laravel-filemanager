...
{
	var current_dir = $('#working_dir').val();
	var working_tree = current_dir.split("/");
	if ( working_tree.length == 2 )
	{
		if ( $('#backButton').length )
		{
			$('#backButton').remove();
			$('#nav-buttons li:first-child').remove();
		}
	} else
	{
		if( !$('#backButton').length )
		{
			$('#nav-buttons').prepend(back_html);
		}
	}

	$('#to-previous').unbind("click");
	...
	$('#thumbnail-display').unbind("click");
